...
/* Filter Functions */
const moviesFilteredByName = (searchTerm) => {
  return movies.filter(x => x.title.toLocaleLowerCase().includes(searchTerm.toLocaleLowerCase()))
}

const moviesFilteredByYear = (searchTerm) => {
  return movies.filter(movie => movie.year >= searchTerm)
}

const standardNameFilters = ['Avenger', 'X-men', 'Princes', 'Batman'];
const standardYearFilters = ['2014']

/* Create & Add Filter Filter Functionalities */
const filterContainer = document.querySelector('.filter-options');

//Add radio button name filters
standardNameFilters.forEach((name) => {
  const radioFilterBtn = document.createElement('input');
  radioFilterBtn.name = 'filter-list-item';
  radioFilterBtn.type = 'radio';
  radioFilterBtn.className = 'radio-filter-btn'
  radioFilterBtn.addEventListener('click', () => {
    posterRender(moviesFilteredByName(name));
  });
  const radioBtnLabel = document.createElement('label');
  radioBtnLabel.innerText = `${name} movies`;
  filterContainer.appendChild(radioFilterBtn);
  filterContainer.appendChild(radioBtnLabel);
});

//Add radio button year filters
standardYearFilters.forEach((year) => {
  const radioFilterBtn = document.createElement('input');
  radioFilterBtn.name = 'filter-list-item';
  radioFilterBtn.type = 'radio';
  radioFilterBtn.className = 'radio-filter-btn'
  radioFilterBtn.addEventListener('click', () => {
    posterRender(moviesFilteredByYear(year));
  });
  const radioBtnLabel = document.createElement('label');
  radioBtnLabel.innerText = `released after ${year}`;
  filterContainer.appendChild(radioFilterBtn);
  filterContainer.appendChild(radioBtnLabel);
});

//Add  input box to search on given text
const textSearchContainer = document.querySelector('.text-search-container');
const searchIconButton = document.createElement('a');
...
searchBox.addEventListener('change', (e) => {
  if(e.target.value.length === 0) {
    ...
    const movs = Object.is(Number(e.target.value), NaN) ? moviesFilteredByName(e.target.value) : moviesFilteredByYear(Number(e.target.value));
    posterRender(movs)
  }
})
searchIconButton.className = 'fa fa-search';
textSearchContainer.append(searchIconButton);
textSearchContainer.append(searchBox);

//Create posters
function posterRender(moviesToDisplay = movies){
  ...
  if(moviesToDisplay.length === 0) {
    ...
    posterContainer.append(noResults)
  } else {
    moviesToDisplay.forEach((movie) => {
      ...
      const posterTile = document.createElement('img');
      posterTile.src = movie.poster;
      posterTile.title = movie.title;
      posterLink.append(posterTile);
      posterContainer.append(posterLink);
    })
  }
}
posterRender()
